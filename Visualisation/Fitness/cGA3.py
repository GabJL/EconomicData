...
"""
    Mostly inspired by : based on the original cGA implemented using opposition-based mutation + applying correction C
    random initialisation
    repair function to ensure K clusters are actually created
    fitness function: sum of squared euclidian distance
    DPX1 crossover
    mutation described in correction "C" : replace the mutated cluster by a random cluster different than the current one
...
def centroids(individual3,dim,data_clust,kk):
    ...
    sam1 = np.array(individual3,dtype="object") # convert the chromosome in numpy array: needed to call predefined funtion "where"
    for ind in range (1,(kk+1)): # browse each cluster
        ...
        for ind1 in sam3: # browse each city
            centroid = np.array(centroid,dtype="object") + np.array(data_clust[ind1],dtype="object")                     
...
def ff(individual,dim,data_clust,kk,center):
    ...
    sam1 = np.array(individual,dtype="object")
    for ind in range (1,(kk+1)): # browse each cluster
...
def distance2(best_indiv,best_indiv_rep,data_clust,k):
    met_2 = 0
    sam1 = np.array(best_indiv_rep,dtype="object")
    for ind in range (1,(k+1)): # browse each cluster     
        dist = np.array([],dtype="object")
        sam2 = np.where(sam1 == ind) # find the cities that belong to the cluster "ind" being processed
        sam3 = sam2[0] 
        for ind1 in sam3:
    ...
    for ind1 in range(1,(k+1)):
        for ind2 in range(1,(k+1)):
            if ind1 != ind2: # avoid the case where we treat the same cluster
                dist_vect = np.array([],dtype="object")
...
def cGA3(pop,dim,it,pc,pm,data_clust,k,N,D):  
    """
    ...
    for ito in range(it): # repeat the cGA's phases "it" times
        best_fit_exe = np.vstack((best_fit_exe,best_fit))       
        ...
        for i in range (0,N): # browse the grid node by node
            for j in range (1,(D+1)):
                ...
                """
                offspring1 = np.array([None] * dim,dtype="object")
                offspring2 = np.array([None] * dim,dtype="object")
                ...
                """
                Mutation: perform random mutation
                """
                for index in range(dim):
                    if random() <= pm: # if condition of mutation satisfied perform the mutation
                        mut_val = offspring[index]
                        while (mut_val == offspring[index]) and (k > 1): # ensures that the chosen random cluster is different from the current one
                            mut_val = np.random.choice(np.arange(1,(k+1)), replace=False, size=(1))                 
                        offspring[index] = mut_val #apply the mutation
                """
    ...
    Delete the padding
    """
    best_fit_exe = np.delete(best_fit_exe,0,0)
    return best_fit_exe;
